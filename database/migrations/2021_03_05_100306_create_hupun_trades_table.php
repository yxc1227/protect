...
class CreateHupunTradesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('hupun_trades', function (Blueprint $table) {
            ...
            $table->decimal('currency_sum',20,3)->default(0)->comment('原始货币金额');
            $table->decimal('discount_fee',20,3)->default(0)->comment('优惠金额');
            ...
            $table->decimal('paid_fee',20,3)->default(0)->comment('实际支付金额');
            ...
            $table->decimal('post_cost',20,3)->default(0)->comment('快递成本,数值可能会变动(如果设置了运费计算规则，订单审核后会根据商品估量和运费规则计算运费成本,称重后根据实际重量重新计算！页面也能直接修改)');
            $table->decimal('post_fee',20,3)->default(0)->comment('邮费');
            ...
            $table->decimal('sum_sale',20,3)->default(0)->comment('总金额,包含优惠');
            ...
            $table->decimal('volume',20,3)->default(0)->comment('体积 单位：立方米');
            $table->decimal('weight',20,3)->default(0)->comment('重量 单位：千克');
