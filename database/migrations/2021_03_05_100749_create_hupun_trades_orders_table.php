...
class CreateHupunTradesOrdersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('hupun_trades_orders', function (Blueprint $table) {
            ...
            $table->decimal('payment',20,3)->default(0)->comment('销售金额');
            $table->decimal('price',20,3)->default(0)->comment('单价(商品标价)');
            $table->decimal('receivable',20,3)->default(0)->comment('应收');
            $table->text('remark')->nullable()->comment('明细备注');
            ...
            $table->decimal('tax_rate',20,3)->default(0)->comment('开票商品税率');
